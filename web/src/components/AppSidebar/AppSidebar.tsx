...
const RouteSidebarContent = () => {
  ...
  if (kind === "home" || kind === "archived" || kind === "explore" || kind === "profile" || kind === "bookmarks") {
    ...
    // the route collection. ponytail: dedicated has_link-scoped stats would widen
 ...
 *
 * The label is a luxury the row affords by width, never by truncation. Seven squares and
 * their gaps cost 208px and the longest label 82px more, so below a 290px row the current page stays a filled square with its tooltip.
 */
const NAV_LABEL_QUERY = "@min-[290px]:";
    ...
    ? [
        {
          id: "unread",
          label: "#unread",
          path: `${collectionPathForLocation(ROUTES.HOME, location.pathname)}?filter=tagSearch:${encodeURIComponent("unread")}`,
          icon: BookOpenIcon,
        ...
        },
        {
          ...
          path: collectionPathForLocation(ROUTES.BOOKMARKS, location.pathname),
          icon: BookmarkIcon,
