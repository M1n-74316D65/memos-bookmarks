	...
	for _, storageType := range []string{"local", "s3"} {
		for _, failure := range []string{"success", "create", "cas"} {
			t.Run(storageType+"/"+failure, func(t *testing.T) {
				...
				setting, err := service.Store.GetInstanceStorageSetting(ctx)
				require.NoError(t, err)
				require.NoError(t, saveAttachmentContent(ctx, service.Profile, service.Store, old, setting, bytes.NewReader(old.Blob)))
				...
				for i := range candidates {
					...
					group.Go(func() {
						errs[i] = saveAttachmentContentWithIsolation(ctx, service.Profile, originalStore, candidates[i], setting, bytes.NewReader(validCoverPNG), true)
					})
				}
				group.Wait()
				for i := range candidates {
