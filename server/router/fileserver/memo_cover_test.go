...
func TestMemoCoverManagedStorageKeepsPrivateCacheHeaders(t *testing.T) {
	for _, kind := range []string{"local", "s3"} {
		t.Run(kind, func(t *testing.T) {
			...
			const coverUID = "managed-cover"
			_, err = svc.CreateAttachment(context.WithValue(ctx, auth.UserIDContextKey, owner.ID), &v1pb.CreateAttachmentRequest{
				AttachmentId: coverUID,
				Attachment:   &v1pb.Attachment{Filename: "link-cover.png", Type: "image/png", Content: encoded.Bytes()},
			})
			require.NoError(t, err)
			memo, err := stores.CreateMemo(ctx, &store.Memo{UID: "managed-memo", CreatorID: owner.ID, Visibility: store.Public,
				Payload: &storepb.MemoPayload{Links: []*storepb.MemoPayload_LinkMetadata{{CoverAttachmentUid: coverUID}}},
			})
			require.NoError(t, err)
			e := echo.New()
			fs.RegisterRoutes(e)
			path := "/file/memos/" + memo.UID + "/covers/" + coverUID
